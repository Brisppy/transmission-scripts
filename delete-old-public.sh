...
#   Gets the time since completion of the torrent
#   IF the torrent is public, progress is 100%, days since completion is greater than MAXAGE:
#       Remove the torrent with data.
...
do
    ...
    # Checks if the torrent is public, completed, finished and is older than MAXAGE.
    if [[ "$PUBLIC" == "Yes" ]] && [[ "$PROGRESS" == "100" ]] && [[ $(date +%s) -gt $(($FINISHDATE + $MAXAGE)) ]]; then
